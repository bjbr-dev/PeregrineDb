...
namespace PeregrineDb.PerformanceTests.PetaPoco
{
    using System;
    ...
    public class Database : IDisposable
    {
                ...
                void CommonConstruct()
        {
            ...
            if (this._connectionString?.IndexOf("Allow User Variables=true") >= 0 && this.IsMySql())
                this._paramPrefix = "?";
        }
        ...
        private bool IsMySql() { return string.Compare(this._providerName, "MySql.Data.MySqlClient", true) == 0; }
        private bool IsSqlServer() { return string.Compare(this._providerName, "System.Data.SqlClient", true) == 0; }
                ...
                try
                {
                    using (var cmd = this.CreateCommand(this._sharedConnection, sql, args))
                    {
                        using (var r = cmd.ExecuteReader())
                        {
                            var l = new List<T>();
                            var pd = PocoData.ForType(typeof(T));
                            var factory = pd.GetFactory<T>(sql + "-" + this._sharedConnection.ConnectionString + this.ForceDateTimesToUtc.ToString(), this.ForceDateTimesToUtc, r);
                            while (r.Read())
                            {
                                l.Add(factory(r));
                            }
        ...
        public IEnumerable<T> Query<T>(string sql, params object[] args) where T : new()
        {
            ...
            {
                using (var cmd = this.CreateCommand(conn.Connection, sql, args))
                {
                    IDataReader r;
                    var pd = PocoData.ForType(typeof(T));
                    try
                    ...
                    var factory = pd.GetFactory<T>(sql + "-" + conn.Connection.ConnectionString + this.ForceDateTimesToUtc.ToString(), this.ForceDateTimesToUtc, r);
                    using (r)
                    {
                        while (true)
                        {
        ...
        {
            T val = this.SingleOrDefault<T>(sql, args);
        ...
        {
            T val = this.FirstOrDefault<T>(sql, args);
            if (!EqualityComparer<T>.Default.Equals(val, default(T)))
                return val;
            else
                throw new InvalidOperationException("The sequence contains no elements");
        ...
        {
            return this.Single<T>(sql.SQL, sql.Arguments);
        }

        public T SingleOrDefault<T>(Sql sql) where T : new()
        {
            return this.SingleOrDefault<T>(sql.SQL, sql.Arguments);
        }

        public T FirstOrDefault<T>(Sql sql) where T : new()
        {
            return this.FirstOrDefault<T>(sql.SQL, sql.Arguments);
        }

        public T First<T>(Sql sql) where T : new()
        {
            return this.First<T>(sql.SQL, sql.Arguments);
        }
        ...
        public object Insert(string tableName, string primaryKeyName, object poco)
        {
            try
            {
                ...
                try
                {
                    using (var cmd = this.CreateCommand(this._sharedConnection, ""))
                    {
                        ...
                        foreach (var i in pd.Columns)
                        {
                            ...
                            AddParam(cmd, i.Value.PropertyInfo.GetValue(poco, null), this._paramPrefix);
                        }
        ...
        {
            var pd = PocoData.ForType(poco.GetType());
            return this.Insert(pd.TableName, pd.PrimaryKey, poco);
        }
                ...
                try
                {
                    using (var cmd = this.CreateCommand(this._sharedConnection, ""))
                    {
                        ...
                        foreach (var i in pd.Columns)
                        {
                            ...
                            if (index > 0)
                                sb.Append(", ");
                            sb.AppendFormat("{0} = {1}{2}", i.Key, this._paramPrefix, index++);
                        ...
                        AddParam(cmd, primaryKeyValue, this._paramPrefix);

                        this._lastSql = cmd.CommandText;
                        this._lastArgs = new object[] { primaryKeyValue };
        ...
        {
            return this.Update(poco, null);
        }

        public int Update(object poco, object primaryKeyValue)
        {
            var pd = PocoData.ForType(poco.GetType());
            return this.Update(pd.TableName, pd.PrimaryKey, poco, primaryKeyValue);
        }

        public int Update<T>(string sql, params object[] args)
        {
            var pd = PocoData.ForType(typeof(T));
            return this.Execute(string.Format("UPDATE {0} {1}", pd.TableName, sql), args);
        }

        public int Update<T>(Sql sql)
        {
            var pd = PocoData.ForType(typeof(T));
            return this.Execute(new Sql(string.Format("UPDATE {0}", pd.TableName)).Append(sql));
        }
        ...
        {
            var pd = PocoData.ForType(poco.GetType());
            return this.Delete(pd.TableName, pd.PrimaryKey, poco);
        }

        public int Delete<T>(string sql, params object[] args)
        {
            var pd = PocoData.ForType(typeof(T));
            return this.Execute(string.Format("DELETE FROM {0} {1}", pd.TableName, sql), args);
        }

        public int Delete<T>(Sql sql)
        {
            var pd = PocoData.ForType(typeof(T));
            return this.Execute(new Sql(string.Format("DELETE FROM {0}", pd.TableName)).Append(sql));
        }
        ...
        {
            var pd = PocoData.ForType(poco.GetType());
            return this.IsNew(pd.PrimaryKey, poco);
        }
        ...
        public void Save(string tableName, string primaryKeyName, object poco)
        {
            ...
            {
                this.Insert(tableName, primaryKeyName, poco);
            }
            else
            {
        ...
        {
            var pd = PocoData.ForType(poco.GetType());
            this.Save(pd.TableName, pd.PrimaryKey, poco);
        }
        ...
        public string LastCommand
        {
            ...
            {
                var sb = new StringBuilder();
                if (this._lastSql == null)
                    return "";
                sb.Append(this._lastSql);
                if (this._lastArgs != null)
                {
                    sb.Append("\r\n\r\n");
                    for (int i = 0; i < this._lastArgs.Length; i++)
                    {
                        sb.AppendFormat("{0} - {1}\r\n", i, this._lastArgs[i].ToString());
                    }
                }
                return sb.ToString();
            }
        ...
        internal class PocoData
        {
            ...
            public PocoData(Type t)
            {
                ...
                this.QueryColumns = string.Join(", ", (from c in this.Columns where !c.Value.ResultColumn select c.Key).ToArray());
            }

            // Create factory function that can convert a IDataReader record into a POCO
            public Func<IDataReader, T> GetFactory<T>(string key, bool ForceDateTimesToUtc, IDataReader r)
            {
                lock (this.PocoFactories)
                {
                    // Have we already created it?
                    if (this.PocoFactories.TryGetValue(key, out object factory))
                        return factory as Func<IDataReader, T>;

                    lock (m_Converters)
                    {
                        // Create the method
                        var m = new DynamicMethod("petapoco_factory_" + this.PocoFactories.Count.ToString(), typeof(T), new Type[] { typeof(IDataReader) }, true);
                        ...
                        for (int i = 0; i < r.FieldCount; i++)
                        {
                            ...
                            if (!this.Columns.TryGetValue(r.GetName(i), out PocoColumn pc))
                                continue;
        ...
        private class ShareableConnection : IDisposable
        {
            ...
            public DbConnection Connection => this._db._sharedConnection;

            private readonly Database _db;

            public void Dispose()
            {
                this._db.CloseSharedConnection();
            }
        }
    ...
    public class Sql
    {
        ...
        {
            return this.Append(new Sql("ORDER BY " + string.Join(", ", (from x in args select x.ToString()).ToArray())));
        }

        public Sql Select(params object[] args)
        {
            return this.Append(new Sql("SELECT " + string.Join(", ", (from x in args select x.ToString()).ToArray())));
        }

        public Sql From(params object[] args)
        {
            return this.Append(new Sql("FROM " + string.Join(", ", (from x in args select x.ToString()).ToArray())));
        }
