...
namespace PeregrineDb.PerformanceTests.Massive
{
    using System;
...
#pragma warning restore 0649
        ...
        {
            this._factory = DbProviderFactories.GetFactory("System.Data.SqlClient");
            /*
        ...
        {
            var result = this._factory.CreateConnection();
            result.ConnectionString = this._connectionString;
            result.Open();
            return result;
        }
        /// <summary>
        ...
        public virtual List<DbCommand> BuildCommands(params object[] things)
        {
            ...
            foreach (var item in things)
            {
                ...
                {
                    commands.Add(this.CreateUpdateCommand(item, this.GetPrimaryKey(item)));
                }
                else
                {
                    commands.Add(this.CreateInsertCommand(item));
                }
            }
        ...
        {
            return this.Execute(new DbCommand[] { command });
        }
        /// <summary>
        ...
        {
            o.ToDictionary().TryGetValue(this.PrimaryKeyField, out object result);
            return result;
        }
        ...
        public virtual DbCommand CreateInsertCommand(object o)
        {
            ...
            if (counter > 0)
            {
                ...
                result.CommandText = string.Format(stub, this.TableName, keys, vals);
            }
            else
            {
        ...
        public virtual DbCommand CreateUpdateCommand(object o, object key)
        {
            ...
            {
                var val = item.Value;
                if (!item.Key.Equals(this.PrimaryKeyField, StringComparison.CurrentCultureIgnoreCase) && item.Value != null)
                {
                    result.AddParam(val);
        ...
        public virtual dynamic Paged(string where = "", string orderBy = "", string columns = "*", int pageSize = 20, int currentPage = 1, params object[] args)
        {
            ...
            var sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER (ORDER BY {2}) AS Row, {0} FROM {3} {4}) AS Paged ", columns, pageSize, orderBy, this.TableName, where);
        ...
        {
            var sql = $"SELECT {columns} FROM {this.TableName} WHERE {this.PrimaryKeyField} = @0";
            var items = this.Query(sql, key).ToList();
