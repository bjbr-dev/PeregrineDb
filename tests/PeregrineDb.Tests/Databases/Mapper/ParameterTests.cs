namespace PeregrineDb.Tests.Databases.Mapper
{
    ...
    public class ParameterTests
    {
        ...
        public void TestTVPWithAdditionalParams()
        {
            ...
            ////        var results = database.Query("get_values", dynamicParameters, commandType: CommandType.StoredProcedure).ToList();
            ...
            ////        try
            ////        {
            ////            database.Execute("DROP PROC get_values");
            ////        }
            ////        finally
            ////        {
            ////            database.Execute("DROP TYPE int_list_type");
        ...
        public void TestSqlDataRecordListParametersWithAsTableValuedParameter()
        {
            ...
            ////        var nums = database.Query<int>("get_ints", new { integers = records.AsTableValuedParameter() },
            ////            commandType: CommandType.StoredProcedure).ToList();
            ////        Assert.Equal(new int[] { 1, 2, 3 }, nums);

            ////        nums = database.Query<int>("select * from @integers", new { integers = records.AsTableValuedParameter("int_list_type") }).ToList();
            ////        Assert.Equal(new int[] { 1, 2, 3 }, nums);

            ////        try
            ////        {
            ////            database.Query<int>("select * from @integers", new { integers = records.AsTableValuedParameter() }).First();
        ...
        public void TestSqlDataRecordListParametersWithTypeHandlers()
        {
            ...
            ////        var nums = database.Query<int>("get_ints", new { integers = records }, commandType: CommandType.StoredProcedure).ToList();
            ////        Assert.Equal(new int[] { 1, 2, 3 }, nums);

            ////        try
            ////        {
            ////            database.Query<int>("select * from @integers", new { integers = records }).First();
            ////            throw new InvalidOperationException();
            ////        }
            ////        catch (Exception ex)
            ////        {
                ...
                try
                {
                    ...
                    for (int i = 0; i < max; Incr(ref i))
                    {
                        try
                        {
                            ...
                            var list = database.Query<int>($"select i from #splits where i in {vals}").AsList();
