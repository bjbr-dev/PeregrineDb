    ...
    public abstract partial class DefaultDatabaseConnectionStatementsTests
    {
            ...
            : DefaultDatabaseConnectionStatementsTests
        {
            ...
            public async Task TestBasicStringUsageAsync()
            {
                ...
                {
                    var query = await database.QueryAsync<string>("select 'abc' as [Value] union all select @value", new { value = "def" }).ConfigureAwait(false);
            ...
            public async Task TestClassWithStringUsageAsync()
            {
                using (var database = BlankDatabaseFactory.MakeDatabase(Dialect.SqlServer2012))
                {
                    var query = await database.QueryAsync<BasicType>("select 'abc' as [Value] union all select @value", new { value = "def" }).ConfigureAwait(false);
